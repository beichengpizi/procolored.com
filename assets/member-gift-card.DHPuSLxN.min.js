import{L as C}from"./theme.B4aNamjw.min.js";import{d as $}from"./ender.NhWCw_5c.min.js";import{i as T}from"./member-api.C5IRMIGH.min.js";import{c as E,e as y}from"./utils.DPbg__7F.min.js";class A extends window.HTMLElement{constructor(){super(),this.api="https://booster.procolored.com",this.lotteryRecordsApi="/procolored/lotteryRecord/getLotteryRecords",this.userInfo=JSON.parse(window.localStorage.getItem("smile_shopify_data"))||{},this.loading=new C,this.initializeComponent(),this.bindEvents()}async initializeComponent(){const{config:e,api:s}=await T();this.MemberApi=s,this.config=e,await this.update()}bindEvents(){this.addEventListener("click",e=>{const s=e.target,t=s.dataset.copy;t&&this.handleCopy(s,t)})}handleCopy(e,s){e.innerHTML="Copy successfully!",e.setAttribute("disabled",!0),E(s),setTimeout(()=>{e.removeAttribute("disabled"),e.innerHTML="Copy"},1e3)}getProductById(e){return this.pointsProducts.find(s=>e===s.id)}parseUTCToLocal(e){if(!e)return"";const s=e.replace(/-/g,"/").replace("T"," ").replace("Z","");let t=new Date(s+" UTC");return isNaN(t.getTime())&&(t=new Date(e)),t}padZero(e){return e<10?"0"+e:e}formatDateToCustomString(e){if(!e||isNaN(e.getTime()))return"";const s=e.getFullYear(),t=this.padZero(e.getMonth()+1),o=this.padZero(e.getDate()),r=this.padZero(e.getHours()),l=this.padZero(e.getMinutes()),m=this.padZero(e.getSeconds());return`${s}/${t}/${o} ${r}:${l}:${m}`}async getUserLotteryRecords(e,s,t){console.log("用户ID:",e),console.log("用户邮箱:",s);try{const o=await fetch(`${this.api}${this.lotteryRecordsApi}?customerEmail=${s}&customerId=${e}&origin=${t}`,{method:"GET",headers:{"Content-Type":"application/json"}});if(!o.ok)throw new Error("网络请求失败");const r=await o.json();if(console.log("查询个人抽奖记录:",r),r.code===200){const l=r.result;return Array.isArray(l)&&l.length>0?l.map((i,L)=>{if(i.prizeType===3){const x=i.prizeTitle||"",u=i.prizeContent||"",b=i.discountTitle||"",h=i.prizeUsageScope||"",w=i.prizePicPc||"",c=i.expireAt||"";let n="",a="",d=!1;if(c!==""){n=this.parseUTCToLocal(c),a=n?this.formatDateToCustomString(n):"";const f=new Date;d=n&&f>n,console.log("UTC折扣券失效时间expireAtUTC :",c),console.log("本地折扣券失效时间expireAt:",a),console.log("当前时间:",this.formatDateToCustomString(f)),console.log("是否已过期:",d)}const g=d?"Expired":i.usageStatus||"Used",p="text-sm m-0",v=g!=="unused"?"m-0":"text-accent m-0",I=g!=="unused"?"bg-gray-50 border-t-gray-400":"border-t-accent bg-yellow-50 lg:hover:shadow-lg lg:hover:scale-105 transition-all";return`
             <div class="bg-gray-50 lg:hover:shadow-lg lg:hover:scale-105 transition-all rounded-lg p-6">
    <div class="flex justify-between">
      <div class="space-y-2">
<h2 class="text-xl font-bold ${v} ">${b}</h2>
      ...
      <div class="w-12 h-12 bg-green-100 flex items-center justify-center rounded-full">
        <img src="${w}" alt="${x}" class="lazyload w-full h-full object-cover">
      </div>
    </div>
  </div>
...
<h2 class="text-xl font-bold text-accent m-0 ">${t.name}</h2>
        ...
        <div class="flex items-center">
          ...
          <p class="ml-4 text-sm text-gray-800 m-0">${this.isExpired(t.created_at,365)?"Expired":$("mm/dd/YYYY",this.getExpiresDate(t.created_at,365))}</p>
        </div>
  ...
  </div>`).join("");this.innerHTML=(this.lotteryRecords||"")+s}isExpired(e,s){const t=new Date,o=this.getExpiresDate(e,s);return t>o}getExpiresDate(e,s){const t=new Date(e);return t.setDate(t.getDate()+s),t}}window.customElements.define("member-gift-card",A);
