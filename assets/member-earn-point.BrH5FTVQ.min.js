import{L as d,M as h,S as u}from"./theme.B4aNamjw.min.js";import{i as y}from"./member-api.C5IRMIGH.min.js";class m extends window.HTMLElement{constructor(){super(),this.loading=new d,this.birthdayModal=new h,this.initComponent(),this.bindEvents()}async initComponent(){const{config:t,api:a}=await y();this.MemberApi=a,this.config=t,this.update()}async update(){this.loading.show({message:"Please wait...",spinnerColor:"text-accent"});try{const{customer_activity_rules:t}=await this.MemberApi.getCustomerActivityRulesAsync(),{customer:a}=await this.MemberApi.getCustomesInfo();this.customer=a,this.activityRules=t,this.renderActivityRules(t)}catch(t){console.error("Error fetching activity rules:",t)}finally{this.loading.hide()}}bindEvents(){this.addEventListener("click",t=>{const{dataset:a}=t.target,{type:e}=a;if(!e)return;const r=this.activityRules.find(i=>i.activity_rule.type===e);switch(e){case"customer_birthday":this.showBirthdayModal();break;default:this.action(r);break}})}renderActivityRules(t){if(!t||t.length===0){this.innerHTML="<div>No activity rules available.</div>";return}const a=t.map(e=>e.activity_rule.is_enabled?`
