...
const COLORCHANNEL_SECRET = "AE19C0F8691FEF37763F32512BCT6S94";
// const booster_base_url = "https://test.procolored.com"; // 测试接口
 const booster_base_url = "https://booster.procolored.com"  // 正式接口
 const colorchannel_base_url = "https://api.colorchannels.com"; // colorchannel正式接口
...
async function generateColorChnanelSign(timestamp) {
  const data = { timestamp };
  const queryStr = httpBuildQuery(data, COLORCHANNEL_SECRET);
  // spark-md5为同步实现，这里可直接返回
  return getMD5String(queryStr);
}
...
async function colorChannelFetch(url, data, options = {}) {
  // options 解析
  const isToJson = options.toJson !== false; // 默认为true
  let method = (options.method || "POST").toUpperCase();
  ...
  if (typeof url === "string") {
    finalUrl = `${colorchannel_base_url}${url}`;
  } else {
    finalUrl = url;
  }
  ...
  if (typeof url === "string") {
    const timestamp = Date.now().toString();
    const sign = await generateColorChnanelSign(timestamp);
    ...
    if (method === "GET" || method === "HEAD") {
      ...
      if (
        ...
        let base = colorchannel_base_url;
        const urlObj = new URL(finalUrl, base);
        Object.entries(data).forEach(([k, v]) => {
          if (Array.isArray(v)) {
            v.forEach((val) => urlObj.searchParams.append(k, val));
