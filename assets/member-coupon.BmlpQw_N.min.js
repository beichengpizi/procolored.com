import{L as v}from"./theme.B4aNamjw.min.js";import{i as x}from"./member-api.C5IRMIGH.min.js";import{e as f}from"./utils.DPbg__7F.min.js";class C extends window.HTMLElement{constructor(){super(),this.loading=new v,this.api="https://booster.procolored.com",this.lotteryRecordsApi="/procolored/lotteryRecord/getLotteryRecords",this.userInfo=JSON.parse(window.localStorage.getItem("smile_shopify_data"))||{}}connectedCallback(){window.Shopify.designMode||this._init()}async _init(){this.loading.show({message:"Please wait...",spinnerColor:"text-accent"});const{config:e,api:s}=await x();this.MemberApi=s,this.config=e,console.log("[_init] this.config:",this.config),console.log("[_init] this.MemberApi:",this.MemberApi),console.log("初始化结束"),console.log("开始执行更新this.update()"),await this.update()}getProductById(e){return this.pointsProducts.find(s=>e===s.id)}parseUTCToLocal(e){if(!e)return"";const s=e.replace(/-/g,"/").replace("T"," ").replace("Z","");let o=new Date(s+" UTC");return isNaN(o.getTime())&&(o=new Date(e)),o}padZero(e){return e<10?"0"+e:e}formatDateToCustomString(e){if(!e||isNaN(e.getTime()))return"";const s=e.getFullYear(),o=this.padZero(e.getMonth()+1),t=this.padZero(e.getDate()),r=this.padZero(e.getHours()),a=this.padZero(e.getMinutes()),c=this.padZero(e.getSeconds());return`${s}/${o}/${t} ${r}:${a}:${c}`}async getUserLotteryRecords(e,s,o){console.log("用户ID:",e),console.log("用户邮箱:",s);try{const t=await fetch(`${this.api}${this.lotteryRecordsApi}?customerEmail=${s}&customerId=${e}&origin=${o}`,{method:"GET",headers:{"Content-Type":"application/json"}});if(!t.ok)throw new Error("网络请求失败");const r=await t.json();if(console.log("查询个人抽奖记录:",r),r.code===200){const a=r.result;return Array.isArray(a)&&a.length>0?a.map((i,S)=>{if(i.prizeType!==3){const w=i.prizeTitle||"",p=i.prizeContent||"",$=i.discountTitle||"",m=i.prizeUsageScope||"",g=i.expireAt||"";let l="",d="",h=!1;if(g!==""){l=this.parseUTCToLocal(g),d=l?this.formatDateToCustomString(l):"";const y=new Date;h=l&&y>l,console.log("UTC折扣券失效时间expireAtUTC :",g),console.log("本地折扣券失效时间expireAt:",d),console.log("当前时间:",this.formatDateToCustomString(y)),console.log("是否已过期:",h)}const n=h?"Expired":i.usageStatus||"Used",u="text-sm m-0",b=n!=="unused"?"m-0":"text-accent m-0";return`
